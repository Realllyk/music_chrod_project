...
class SongsMapper:
    ...
    def insert(song_data):
        ...
        try:
            with conn.cursor() as cursor:
                sql = """
                    INSERT INTO songs
                    (title, artist_id, category, duration, source, source_id, audio_path, melody_path, chord_path, melody_key, chord_key, status, session_id)
                ...
                cursor.execute(sql, (
                    ...
                    song_data.get('melody_key'),
                    song_data.get('chord_key'),
    ...
    def update(song_id, song_data):
        ...
        try:
            with conn.cursor() as cursor:
                fields = []
                values = []
                for key in ['title', 'artist_id', 'category', 'duration', 'source', 'source_id', 'audio_path', 'session_id', 'melody_path', 'chord_path', 'melody_key', 'chord_key', 'status']:
    ...
    @staticmethod
    def update_melody_key(song_id, melody_key):
        return SongsMapper.update(song_id, {'melody_key': melody_key})

    @staticmethod
    def update_chord_key(song_id, chord_key):
        return SongsMapper.update(song_id, {'chord_key': chord_key})
